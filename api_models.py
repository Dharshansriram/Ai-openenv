...
if _PYDANTIC:
    ...
    class ResetRequestModel(BaseModel):
        scenario: ScenarioEnum = PField(default=ScenarioEnum.medium)
        seed: int = PField(default=42)
        session_id: Optional[str] = PField(default=None)
    ...
    class ActionRequestModel(BaseModel):
        model_config = ConfigDict(
            json_schema_extra={
                "example": {
                    "action_type":"Required",   # here action should decide by us
                    "developer_id":"",    # here developer is optional for pairing it is not required
                    "primary_developer_id":"",  #it is for pair programming
                    "secondary_developer_id":"", #this too
                    "new_priority":"",  # then it is for focusing priority lvl
                    "split_ratio": '' #for subtask
                }
            }
        )

        action_type: Optional[ActionTypeEnum]
        task_id: Optional[str] = None
        developer_id: Optional[str] = None
        primary_developer_id: Optional[str] = None
        secondary_developer_id: Optional[str] = None
